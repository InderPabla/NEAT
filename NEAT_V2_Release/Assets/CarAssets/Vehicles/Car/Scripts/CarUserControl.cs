...
public class CarUserControl : MonoBehaviour, IAgentTester
{
    ...
    private bool drawLineRen = false;
    ...
    public void UpdateNet()
    {
        ...
        for (int i = 0; i < numberOfSensors; i++)
        {
            ...
            sightHit[(i * 2)] = -1f;
            sightHit[(i * 2) + 1] = 0f;
            if (rayHit != null && rayHit.Length > 0 && rayHit[0].collider != null) //&& rayHit[i].collider.isTrigger == false)
            {
    ...
    {
        Renderer[] children = transform.FindChild("SkyCar").GetComponentsInChildren<Renderer>();
